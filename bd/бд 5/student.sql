...
ADD CONSTRAINT fkey_mark_su FOREIGN KEY (subject_id) REFERENCES subjects(id);
...
CREATE OR REPLACE FUNCTION autocorrect_mark()
RETURNS trigger AS
$body$
BEGIN
	CASE 
		WHEN  new.mark between 2 and 5
		THEN
			INSERT INTO marks VALUES (new.*)
		ELSE
			RAISE EXCEPTION 'incorrect value of mark --> %', mark
...
ON marks
FOR EACH ROW
EXECUTE PROCEDURE autocorrect_mark();
